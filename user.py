...
async def handle_add_to_basket(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    try:
        ...
        # Query using original price, and randomize selection
        # ***** MODIFICATION FOR RANDOMIZED DROP SELECTION *****
        c.execute("SELECT id FROM products WHERE city = ? AND district = ? AND product_type = ? AND size = ? AND price = ? AND available > reserved ORDER BY RANDOM() LIMIT 1", (city, district, p_type, size, float(original_price)))
        # ***** END MODIFICATION *****
...
async def handle_pay_single_item(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    try:
        ...
        c.execute("BEGIN EXCLUSIVE")
        # ***** MODIFICATION FOR RANDOMIZED DROP SELECTION *****
        c.execute("SELECT id, name, price, size, product_type FROM products WHERE city = ? AND district = ? AND product_type = ? AND size = ? AND price = ? AND available > reserved ORDER BY RANDOM() LIMIT 1", (city, district, p_type, size, float(original_price)))
        # ***** END MODIFICATION *****
